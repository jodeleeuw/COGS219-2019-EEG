...
```{r}
plot(M ~ factor(distractor.condition), data=p3b.distractor.anova.data)
```
...
```{r}
eeg.data.correct.incorrect <- read_csv('data/tidy/eeg-data-correct-incorrect-tidy.csv')
```
...
Same sensors as previous N2. Time window 250-350.

t-test between left/right hemisphere on the difference wave between L/R.

one sample t-test against 0 for difference wave.


#### PP

same sensors as ??. Unclear!

410-650ms after distractor.
